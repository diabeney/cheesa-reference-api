...
import { ReferenceShape, RespondToReference, submitRequestedReference, updatePayment } from "../constants/constants";
import { ZodError } from "zod";
import { Types } from "mongoose";
import { getLecturerById } from "../db/user";
import { getUsersReferenceByRole, getReferenceById, RequestReference, updateReferenceById } from "../db/reference";
import { RequestReference as RefReqObject, ReferenceResponse } from "../types/types";
...
interface AuthQueryRequest extends Request<unknown, unknown, unknown, QueryFields> {
  userPayload?: TokenPayload;
}

async function handleRequestReference(req: Request<unknown, unknown, RefReqObject>, res: Response) {
  ...
  const { quantity, lecturerId, graduateId, purposeOfReference, requests } = formObj;

  if (quantity !== requests.length) return res.status(400).json(ErrorMsg(400, "Number of requests does not match quantity"));
  ...
  try {
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      const { destination, expectedDate, address, modeOfPostage, description } = request;
...
async function handleUpdateReference(req: Request, res: Response) {
  const {
    params: { id },
  } = req;
  ...
  const validatedPayload = validateObject(updatePayload, updatePayment);

  if (validatedPayload instanceof ZodError) {
    const { message } = validatedPayload.issues[0];
    return res.status(STATUS.BAD_REQUEST.code).json(ErrorMsg(400, message));
  }

  try {
    await updateReferenceById(validatedPayload.refId, { transactionStatus: validatedPayload.transactionStatus });
...
export { handleRequestReference, handleViewReference, handleUpdateReference };
