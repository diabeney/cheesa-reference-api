...
async function handleSignUp(
	...
	try {
		const lectureEmail = await LecturerEmails.findOne({ email: email });
		const userExists = await getUserByEmail(email);

		if (role === "lecturer" && !lectureEmail) {
			return res.status(400).json(ErrorMsg(400, "Lecturer email not found"));
		}
...
async function handleLogin(
	...
	try {
		...
		//check if user is verified then log in the user
		if (!user.isVerified) {
			return res
				.status(401)
				.json(
