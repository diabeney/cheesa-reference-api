...
const SignUpShape = z.object({
  ...
  referenceNumber: z.string({ required_error: "Reference Number is required" }).min(8, "Reference should be at least 8 characters"),
  indexNumber: z.string().min(7, "Index number should be at least 7 characters").optional(),
...
const ReferenceShape = z.object({
  ...
  quantity: z.number({ required_error: "Quantity is required" }).int().min(1, "At least 1 request is required"),
});
...
const updatePayment = z.object({
  refId: z.custom<Types.ObjectId>(),
  transactionStatus: z.enum(["pending", "paid"]),
});
