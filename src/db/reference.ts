...
const RequestReference = async (payload: IReferenceRequest) => await new Reference(payload).save();
...
const getUsersReferenceByRole = async (id: string, role: "lecturer" | "graduate") => {
  if (role === "graduate") {
    const result = await Reference.find({ graduateId: id })
      .populate({
        path: "graduateId lecturerId",
        select: "indexNumber referenceNumber programme entryYear graduationYear",
        model: Users,
      })
      .sort({ _id: -1 })
      .then((reference) => {
  ...
  const result = await Reference.find({ lecturerId: id })
    .populate({
      ...
      select: "referenceNumber indexNumber firstName lastName programme entryYear graduationYear",
      model: Users,
    })
    .sort({ _id: -1 })
    .then((reference) => {
...
const updateReferenceById = async (id: Types.ObjectId, payload: any) => await Reference.findOneAndUpdate({ _id: id }, payload, { new: true });

export { RequestReference, getUsersReferenceByRole, updateReferenceById, getReferenceById };
